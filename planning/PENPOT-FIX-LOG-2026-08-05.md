...
1. **nginx response cache.** Penpot's plugin iframe fetched `/sonic-studio-b64/01-design-system.b64` but nginx was returning the OLD Editorial Zine b64 (582,260 chars) instead of the cassette-mesh b64 (1,728,540 chars). Even after `nginx -s reload`, the cache persisted. **Solution: append a unique `?v=N` cache-buster to every fetch URL.**
...
2. Copied to nginx `/tmp/sonic-studio-b64/` and reloaded nginx.
...
4. Created a new design token set `sonic-studio/cassette-mesh` with 24 tokens (10 colors in hex, 6 font sizes, 5 spacings, 2 border radii). Used hex instead of hsl because Penpot tokens reject hsl format.
...
1. Open the file `sonic-studio-design` in Penpot.
...
4. Delete the old `sonic-studio/design-system` token set (the new `sonic-studio/cassette-mesh` set is the one to use).
