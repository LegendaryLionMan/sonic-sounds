# Sonic Studio — Catch-up Guide

> **Purpose**: this file is the single source of truth a second LLM (or a human returning after a break) can read to fully understand what Sonic Studio is, why each design decision was made, and how to keep working on it without breaking things.
>
> Last updated: 2026-09-04 (the day the project was renamed from album-studio → Sonic Studio and pushed to https://github.com/LegendaryLionMan/sonic-studio)
...
- **All data lives in SQLite at `.meta/sonic-studio.db`** (override via `SONIC_STUDIO_DB_PATH` env var)
...
| Theme | Mood | Use |
|---|---|---|
| **Mixtape '85** (default) | Warm yellow + cyan on near-black | The album-studio identity — locked 2026-08-05 |
...
**All env vars use `SONIC_STUDIO_` prefix** (was `ALBUM_STUDIO_` before the rename). The full list:
...
| `SONIC_STUDIO_MMX_CMD` | (none) | Override path to `mmx` CLI for M-layer generation |
| `SONIC_STUDIO_E2E_BASE` | `http://127.0.0.1:8793` | Daemon URL for e2e tests |
| `SONIC_STUDIO_E2E_PORT` | `8793` | Port the e2e runner starts on |
| `SONIC_STUDIO_E2E_HEADLESS` | `1` | Run Playwright in headless mode for e2e |
...
- Commit: `16f3218`
- Created https://github.com/LegendaryLionMan/sonic-studio
- Renamed from `album-studio` to `Sonic Studio` (160 files changed)
...
- All env vars prefixed `SONIC_STUDIO_` (was `ALBUM_STUDIO_`)
...
Sonic Studio is a local-first Python+JS music-album design + production studio. A user fills out a 26-question intake form, the brief locks into SQLite, and 13 build layers execute in sequence via click-to-invoke buttons, producing a complete album package (10 tracks, ISRC codes, ID3v2.3 tags, 9 cover designs). The header player (cassette visual, 15 keyboard shortcuts, 6 themes) persists across all pages. The user Francisco (LegendaryLionMan) iterates daily and has built ~19 days of polish into the player specifically. Local-first, no cloud, MIT-licensed. The current focus is visual polish + power-user features (keyboard shortcuts, A/B loop, sleep timer, playback speed).
