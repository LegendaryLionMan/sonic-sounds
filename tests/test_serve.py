...
Test isolation: each class run uses a fresh tempdb via SONIC_SOUNDS_DB_PATH
so the live .meta/sonic-sounds.db is never mutated.
"""
import json
import os
import sys
...
class TestServe(unittest.IsolatedAsyncioTestCase):
    ...
    Each class run uses a fresh tempdb (set via SONIC_SOUNDS_DB_PATH)
    so the live .meta/sonic-sounds.db is never mutated.
    """
    ...
    def setUpClass(cls):
        ...
        cls.tmpdir = Path(tempfile.mkdtemp(prefix="sonic-sounds-test-serve-"))
