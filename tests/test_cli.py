...
Each test uses a fresh tempdb via the SONIC_SOUNDS_DB_PATH env var so the
live .meta/sonic-sounds.db is never touched. The setUpClass creates one
...
SONIC_SOUNDS_DB_PATH for the current process BEFORE importing the db
...
# resolved at import time and would capture the live .meta/sonic-sounds.db.
...
class TestCli(unittest.TestCase):
    ...
    Runs against a per-class tempdb so the live .meta/sonic-sounds.db is
    ...
    def setUpClass(cls):
        ...
        # SONIC_SOUNDS_DB_PATH from the environment.
    ...
    def _subprocess_env(self) -> dict:
        ...
        env = os.environ.copy()
        env["SONIC_SOUNDS_DB_PATH"] = str(self.tempdb)
        return env
    ...
    def test_chat_cli(self):
        ...
        Uses tempdb (via SONIC_SOUNDS_DB_PATH in the subprocess env) and
        ...
        # setUpClass set SONIC_SOUNDS_DB_PATH and dropped the cached modules
