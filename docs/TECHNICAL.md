# sonic-sounds · Technical Implementation Reference

> The complete architecture of sonic-sounds v3.4 (Mixtape '85 era),
...
Single daemon process per sonic-sounds project. Bound to `127.0.0.1:8765` by default. Background threads:
...
def invoke_music(action: str, args: dict, *, mmx_cmd: str = None) -> InvokeOutcome:
    ...
    mmx_cmd = mmx_cmd or os.environ.get("SONIC_SOUNDS_MMX_CMD") or r"C:\Users\lion_\AppData\Roaming\npm\mmx.cmd"
...
| `SONIC_SOUNDS_MMX_CMD` | `~/AppData/Roaming/npm/mmx.cmd` | mmx CLI path |
