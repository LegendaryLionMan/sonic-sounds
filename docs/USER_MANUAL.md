...
Album-studio is the **web UI** companion to the existing `music-album-planning-questionnaire` skill. The skill drives the **content** (what to ask, what reference bands to surface); sonic-sounds drives the **state** (which questions are answered, which layers are running, which files are written).
...
```mermaid
flowchart TD
    Start([User opens sonic-sounds]) --> Pick[Pick or create album]
...
Page captures live in [`docs/assets/screenshots/`](assets/screenshots/README.md) (mirrored to `~/OneDrive/Hermes/Agents/planning/sonic-sounds/docs/assets/screenshots/` per R7). The naming convention is:
...
- **Database:** SQLite at `<project_root>/.meta/sonic-sounds.db` (or `SONIC_SOUNDS_DB_PATH`).
