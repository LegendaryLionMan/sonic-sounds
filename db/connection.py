...
Module-level helpers:
  DEFAULT_DB_PATH — the canonical .meta/sonic-sounds.db (Q25 + R10)
...
def default_db_path() -> Path:
    ...
    toggle the SONIC_SOUNDS_DB_PATH environment variable mid-process
    see the updated path. The env var takes precedence; otherwise the
    project-root .meta/sonic-sounds.db is used.

    Per Q25 + R10: the canonical live database lives in
    .meta/sonic-sounds.db inside the project root.
    """
    override = os.environ.get("SONIC_SOUNDS_DB_PATH")
    if override:
        return Path(override).resolve()
    return _PROJ_ROOT / ".meta" / "sonic-sounds.db"
...
# of the default path. Tests that mutate SONIC_SOUNDS_DB_PATH after
