...
def find_intake_path(album_dir: Path) -> Path | None:
    ...
    3. ~/Documents/Projects/sonic-sounds/intake-data/<slug>.json (the
       canonical sonic-sounds project root — most common case)
    """
    slug = album_dir.name
    candidates = [
        album_dir / "intake-data" / f"{slug}.json",
        album_dir.parent / "intake-data" / f"{slug}.json",
        Path(r"C:\Users\lion_\Documents\Projects\sonic-sounds\intake-data") / f"{slug}.json",
    ]
    for c in candidates:
        if c.exists():
            return c
